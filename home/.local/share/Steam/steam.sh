...
function check_requirements()
{
	...
	if [ -z "$check_requirements_bin" ]; then
		echo "Can't find 'steam-runtime-check-requirements', continuing anyway"
	else
		...
		if (( $status == 71 )); then
			...
			echo "Steam client's requirements are satisfied"
		fi
	fi
}
...
if [ "$STEAM_RUNTIME" -a "$STEAM_RUNTIME" != "0" ]; then
	...
	if unpack_runtime; then
		if [ ! -x "$STEAM_RUNTIME/setup.sh" ]; then
			echo "internal error: $STEAM_RUNTIME/setup.sh is missing, this runtime is invalid or corrupted"
			exit 1
		fi
		export PATH="$("$STEAM_RUNTIME/setup.sh" --print-bin-path):$PATH"

		if [ ! -x "$STEAM_RUNTIME/run.sh" ]; then
			echo "internal error: $STEAM_RUNTIME/run.sh is missing, this runtime is invalid or corrupted"
			exit 1
		fi
...
if [ "$STEAM_DEBUGGER" == "gdb" ] || [ "$STEAM_DEBUGGER" == "cgdb" ]; then
	...
	echo set index-cache on >> "$ARGSFILE"
	echo show index-cache >> "$ARGSFILE"
	...
	echo "WARNING: Using default/fallback debugger launch"
	echo $STEAM_DEBUGGER "$STEAMROOT/$STEAMEXEPATH" "$@"
